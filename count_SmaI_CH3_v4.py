...
def main():
    ...
    ###### Open file, first make sure that bam file is sorted and indexed  ###### (probably this will be moved into own function)
    ...
    ### Check if Sma sides are already stored as pickle, if not crete it for future use ### put int  function SmaI_DB = load_SmaI_DB()
    if os.path.isfile('SmaI_sides_keys.pickle'):
        with open('SmaI_sides_keys.pickle', 'rb') as handle:
            SmaI_DB = pickle.loads(handle.read())
    else:
        f=open("SmaI_sides_keys.txt","r")
        smai_sides=[row.strip().split('\t') for row in f]
        f.close()
        SmaI_DB={key:[value,{'M':0},{'U':0}] for key,value in smai_sides} ### getting too complicated probably this should become the class object with methylation status method
        with open('SmaI_sides_keys.pickle', 'wb') as handle:
    ...
    for (counter, read) in enumerate(samfile.fetch('chr21', 10000000, 10200000)):
        ...
        SmaI_side_key="_".join([read.tid,SmaI_side]
